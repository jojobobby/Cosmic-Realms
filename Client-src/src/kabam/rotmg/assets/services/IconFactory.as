...
public class IconFactory {
    ...
    private static function cropAndGlowIcon(_arg1:BitmapData):BitmapData {
        ...
        return (BitmapUtil.cropToBitmapData(_arg1, 10, 10, (_arg1.width - 20), (_arg1.height - 20),0xffffff));
    }
    ...
    public function makeIconBitmap2(id:int, setName:String):Bitmap {
        var _local2:BitmapData = AssetLibrary.getImageFromSet(setName, id);
        _local2 = TextureRedrawer.redraw(_local2, (310 / _local2.width), true, 0);
        return (new Bitmap(_local2));
    }

    public function makeIconBitmapFromPotions():Bitmap {
        var x:Array = [52, 53, 54, 48, 49, 55, 38, 39];
        var result:int = x[int(Math.floor(Math.random() * x.length))];
        var _local2:BitmapData = AssetLibrary.getImageFromSet("lofiObj2", result);
        _local2 = TextureRedrawer.redraw(_local2, (310 / _local2.width), true, 0);
        return (new Bitmap(_local2));
    }
