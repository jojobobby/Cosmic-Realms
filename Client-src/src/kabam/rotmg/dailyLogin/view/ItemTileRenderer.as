...
public class ItemTileRenderer extends Sprite {
    ...
    private function addToolTipToTile(itemTile:ItemTile):void
    {
        var itemType:String;
        if (this.itemId > 0)
        {
        ...
        {
            if ((itemTile is EquipmentTile))
            {
                itemType = ItemConstants.itemTypeToName((itemTile as EquipmentTile).itemType);
            }
            else
            {
                itemType = "item.toolTip";
            }
            this.tooltip = new TextToolTip(0x363636, 0x9B9B9B, null, TextKey.ITEM_EMPTY_SLOT, 200, {"itemType": TextKey.wrapForTokenResolution(itemType)});
        }
        this.tooltip.attachToTarget(itemTile);
        var injector:Injector = StaticInjectorContext.getInjector();
        var signal:ShowTooltipSignal = injector.getInstance(ShowTooltipSignal);
        signal.dispatch(this.tooltip);
    }
