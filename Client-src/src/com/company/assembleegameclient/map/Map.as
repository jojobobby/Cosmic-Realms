...
public class Map extends AbstractMap {
    ...
    public function correctMapView(_arg_1:Camera):Point
    {
        ...
        } else
        {
            if (stage.scaleMode == StageScaleMode.NO_SCALE)
            {
                ...
                x = ((-(_local_2.x) * 800) / (WebMain.sWidth / _local_7));
                y = ((-(_local_2.y) * 600) / (WebMain.sHeight / _local_7));
            }
        }
    ...
    override public function draw(camera:Camera, currentTime:int):void {
        ...
        {
            x = ((-(rect.x) * 800) / WebMain.sWidth * Parameters.data_["mscale"]);
            y = ((-(rect.y) * 600) / WebMain.sHeight * Parameters.data_["mscale"]);
        } else
        {
            x = -(rect.x);
            y = -(rect.y);
