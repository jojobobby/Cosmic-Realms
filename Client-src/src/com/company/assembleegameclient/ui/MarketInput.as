...
public class MarketInput extends Sprite
{
    ...
    public function MarketInput(name:String, isPassword:Boolean, error:String, fontSize:int = 20, width:int = 238, height:int = 30)
    {
        super();

        if (name != "") {
            this.nameText_ = new BaseSimpleText(fontSize - 2, 11776947, false, 0, 0);
        ...
        this.inputText_ = new BaseSimpleText(fontSize,11776947,true,238,30);
        ...
        graphics.drawRect(0,this.inputText_.y,width, height);
        graphics.endFill();
        graphics.lineStyle();
        this.inputText_.addEventListener(Event.CHANGE,this.onInputChange);
        this.errorText_ = new BaseSimpleText(fontSize - 8,16549442,false,0,0);
