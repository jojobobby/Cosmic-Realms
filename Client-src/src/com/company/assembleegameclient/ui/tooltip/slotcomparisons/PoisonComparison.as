...
public class PoisonComparison extends SlotComparison {
    ...
    override protected function compareSlots(itemXML:XML, curItemXML:XML):void {
        ...
        if ((((activate.length() == 1)) && ((otherActivate.length() == 1)))) {
            ...
            amount = Number(activate[0].@amount);
            otheramount = Number(otherActivate[0].@amount);
            if (!amount || !amount >= 1) {
                amount = 0
            }
            avg = (((0.33 * damage) + (0.33 * radius)) + (0.33 * duration)+ (0.33 * amount));
            otherAvg = (((0.33 * otherDamage) + (0.33 * otherRadius)) + (0.33 * otherDuration)+ (0.33 * otheramount));
