...
public class Player extends Character {
    ...
    public static const MS_BETWEEN_TELEPORT:int = 5000;
    ...
    private static const ADMIN_OFFSET_MATRIX:Matrix = new Matrix(1, 0, 0, 1, 0, 1);
    ...
    public var CriticalDmg_:int = 0;
    public var CriticalHit_:int = 0;
    public var CriticalDmgMax_:int = 0;
    public var CriticalHitMax_:int = 0;
    public var CriticalDmgBoost_:int = 0;
    public var CriticalHitBoost_:int = 0;
    ...
    public function Player(_arg1:XML) {
        ...
        this.CriticalDmgMax_ = int(_arg1.CriticalDmg.@max);
        this.CriticalHitMax_ = int(_arg1.CriticalHit.@max);
    ...
    public static function fromPlayerXML(_arg1:String, _arg2:XML):Player {
        ...
        _local5.CriticalDmg_ = int(_arg2.CriticalDmg);
        _local5.CriticalHit_ = int(_arg2.CriticalHit);
    ...
    public function teleportTo(_arg1:Player):Boolean {
        ...
        else if (!this.isTeleportEligible(_arg1)) {
    ...
    override public function update(currentTime:int, deltaMS:int):Boolean {
        ...
        if (this.relMoveVec_ != null) {
            ...
            if (_pushX != 0 || _pushY != 0) {
                moveVec_.x = moveVec_.x - _pushX;
                moveVec_.y = moveVec_.y - _pushY;
            }
    ...
    private var _pushX:Number;
    private var _pushY:Number;
    ...
    public function onMove():void {
        ...
        if (square_.props_.push_) {
            _pushX = square_.props_.animate_.dx_ / 1000;
            _pushY = square_.props_.animate_.dy_ / 1000;
        } else {
            _pushX = 0;
            _pushY = 0;
        }
    ...
    override protected function makeNameBitmapData():BitmapData {
        ...
        _local3 = _local2.make(_local1, 16, this.getNameColor(), true, NAME_OFFSET_MATRIX, true);
        _local3.draw(FameUtil.numStarsToIcon(this.numStars_, false), RANK_OFFSET_MATRIX);
        return (_local3);
    }
    ...
    override public function draw(_arg1:Vector.<IGraphicsData>, _arg2:Camera, _arg3:int):void {
        ...
        {
            if (!this.starred_ && Parameters.data_.hideLockList)
            {
                return;
            }
        }
    ...
    private function getMoveSpeed():Number {
        ...
        if (((isFrozen()))) {
            _local1 = (_local1 * 0.50);
        }
        if (((isSwift()))) {
            _local1 = (_local1 * 1.25);
        }
        return ((_local1 * this.moveMultiplier_)* (!!this.isWalking?Parameters.data_.walkSpeed:1));
    }
    ...
    override protected function getTexture(camera:Camera, currentTime:int):BitmapData {
        ...
        if (plrTex == null) {
            plrTex = GlowRedrawer.outlineGlow(tex, this.glowColor_,1.4, true, 3, true);
            texturingCache_[tex] = plrTex;
        }

        if ((isPaused() || isStasis() || isPetrified()) && (commune == null)) {
    ...
    private function doShoot(_arg1:int, _arg2:int, _arg3:XML, _arg4:Number, _arg5:Boolean):void {
        ...
        while (_local10 < _local6) {
            ...
            _local12.setDamage(_local16, critMulti());
    ...
    public function critMulti() : Number
    {
        ...
        var critMult:Number = (100 + this.CriticalDmg_) / 100.0;

        if (randChance < critChance) {
            criticalMultiplier_ *= critMult;
        }
        if (weapon.Critical)
        {
            if (erandChance < 1) {
    ...
    public function sToW(n:Number, n2:Number) : Point
    {
        var angle:Number = Parameters.data_.cameraAngle;
        ...
        n = n / 50;
        n2 = n2 / 50;
        var number:Number = n * cos - n2 * sin;
        var number2:Number = n * sin + n2 * cos;
        return new Point(this.x_ + number,this.y_ + number2);
    }
