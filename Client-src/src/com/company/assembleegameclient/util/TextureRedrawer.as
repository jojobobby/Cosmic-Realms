...
public class TextureRedrawer {
    ...
    public static function redraw(tex:BitmapData, size:int, padBottom:Boolean, glowColor:uint, useCache:Boolean = true, sMult:Number = 5, glowMult:Number = 1.4,itemGlowStrenght:Number=1.1):BitmapData {
        var hash:* = getHash(size, padBottom, glowColor, sMult);
        if (useCache && isCached(tex, hash)) {
            return redrawCaches[tex][hash];
        }
        var glow:BitmapData = resize(tex, null, size, padBottom, 0, 0, sMult);
        if (!Parameters.data_.FPSMode) {
            glow = GlowRedrawer.outlineGlow(glow, glowColor, glowMult, useCache,itemGlowStrenght);
        }
        if (useCache) {
            cache(tex, hash, glow);
        }
