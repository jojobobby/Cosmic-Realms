...
public class SavedCharactersList extends Event {
    ...
    private function parseVaultData(xml:XML):void
    {
        var vault:String;
        var ints:Vector.<int>;
        var ints1:Vector.<int>;
        var i:int;
        var i1:int;
        if (xml.hasOwnProperty("Vault"))
        {
            vaultItems = new Vector.<Vector.<int>>();
            for each (vault in xml.Vault.Chest)
            {
                ints = new Vector.<int>();
                ints1 = ParseUtils.parseIntList(vault);
                if (ints1 != null)
                {
                    for each (var i2:int in ints1)
                    {
                        ints.push(i2);
                    }
                }
