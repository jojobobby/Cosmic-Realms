...
namespace common
{
    public class Database : IDisposable
    {
        ...
        public string GetAccountsByIP(string ip)
        {
            if (String.IsNullOrEmpty(ip))
                return "";
            var acc = _db.HashGet("ips", ip).ToString();
            var split= acc.Split('[');
            split = split[1].Split(']');
            return split[0];
        }
        public void AddVaults()
        {
           ...
           //     var acc = new DbAccount(_db, i);
            ...
            //    {
            //        _db.HashSetAsync(acc.Key, "vaultCount", 200, flags: CommandFlags.FireAndForget);
             //   }
            }
        }
        ...
        public bool doesAccountLinkExist(string discordAccount)
        {

            /*if (!_db.HashSet("discordAccountsUsed", discordIds, "{}", When.NotExists))
                return false;*/ //add redis object in db with name discordAccountsUsed, create field discordIds
            return true;
        }
        ...
        public LoginStatus Verify(string uuid, string password, out DbAccount acc, string sCode = "", bool isVerifying = false)
        {
            acc = null;
            ...
            if (!String.IsNullOrEmpty(acc.DiscordAuthAccount) && isVerifying == true)
            {
                if (!String.IsNullOrEmpty(sCode)) {
                    if (sCode != acc.SecretCode) {
                        acc.SecretCode = generateCode();
                        acc.FlushAsync();
                        return LoginStatus.WrongCode;
                    }
                    acc.SecretCode = generateCode();
                    acc.FlushAsync();
                }
        ...
        public RegisterStatus Register(string uuid, string password, bool isGuest, string ip, out DbAccount acc)
        {
            var newAccounts = _resources.Settings.Accounts;

            acc = null;

            //var json = (string)_db.HashGet("ips", ip);
            //if (json != null)
            //    return RegisterStatus.IP;
            ...
            //packs that you dont want new accounts getting.
        ...
        public void AddMarketData(DbAccount acc, ushort itemType, int sellerId, string sellerName,
            int price, int slotType)
        {
            ...
            data.Currency = CurrencyType.Fame;
            data.Price = price;
        ...
        public void ResetFame()
        {
            ...
            {
                var acc = new DbAccount(_db, i);
                if (acc.Fame > 0 || acc.TotalFame > 0)
                {
                    Log.Info($"{acc.Name}:{acc.IP} has {acc.Fame} fame (total of {acc.TotalFame}).");
        ...
        public void BanIp2(int ip, string notes = "")
        {
            var abi = new DbIpInfo(_db, ip.ToString())
            {
                Banned = true,
                Notes = notes
            };
        ...
        public bool AddAchievement(DbAccount acc, int achievementId)
        {
            var aList = acc.Achievements.ToList();
            if (aList.Contains(achievementId))
                return false;
            aList.Add(achievementId);
            acc.Achievements = aList.ToArray();
            acc.FlushAsync();
            return true;
        }
    }
