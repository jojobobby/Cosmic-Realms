...
namespace wServer.realm
{
    ...
    public class Inventory : IEnumerable<Item>
    {
        ...
        public InventoryTransaction CreateTransaction() => new InventoryTransaction(_parent);
        ...
        public int[] GetAvailableInventorySlots(int amount)
        {
            using (TimedLock.Lock(_invLock))
            {
                var plr = _parent as Player;
                var freeSlots = 0;
                var slots = new int[amount - 1];
                if (plr != null)
                {
                    for (var i = 4; i < 12 || (plr.HasBackpack && i < plr.Inventory.Length); i++)
                        if (_items[i] == null)
