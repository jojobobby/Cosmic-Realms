    ...
    public class PortalMonitor
    {
        ...
        public bool AddPortal(int worldId, Portal portal = null, Position? position = null, bool announce = true)
        {
            ...
            using (TimedLock.Lock(_worldLock))
            {
                ...
                if (portal == null)
                {
                    pos = _world.GetRegionPosition(TileRegion.Realm_Portals).GetValueOrDefault();
                    ...
                    else if (world is Moon)
                        portal = new Portal(_manager, 0x4312, null)
                        {
                            WorldInstance = world,
                            Name = world.GetDisplayName() + " (0)"
                        };
                    else
                        portal = portal = new Portal(_manager, 0x6238, null)
                        {
                            WorldInstance = world,
                            Name = world.GetDisplayName()+ " (0)"
                        };

                    portal.SetDefaultSize(120);
                }
                ...
                if (announce)
                {
                    foreach (var w in _manager.Worlds.Values)
                    {
                        foreach (var p in w.Players.Values)
                        {
                            ...
                            p.SendInfo(string.Format("A rift to {0} has been opened in the solar system{1}.",
