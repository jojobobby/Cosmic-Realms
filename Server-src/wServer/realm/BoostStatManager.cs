...
namespace wServer.realm
{
    class BoostStatManager
    {
        ...
        private void ApplyLBBonus(InventoryChangedEventArgs e)
        {
            for (int i = 4; i < _player.Inventory.Length; i++)
            {
                if (_player.Inventory[i] == null)
                    continue;

                foreach (var b in _player.Inventory[i].LBAmount)
                    IncrementLBBoost(b.Value);

            }
        }

        private void ApplyPercBonus(InventoryChangedEventArgs e)
        {
            for (var i = 0; i < 4; i++)
            {
                if (_player.Inventory[i] == null)
                ...
                foreach (var b in _player.Inventory[i].StatsBoostPerc)
                    if (b.Value != 0)
                    {
                        ...
                        IncrementPercBoost((StatsType)b.Key, (int)((_parent.Base[index] + _boost[index]) * (b.Value / 100.0)));
                    }
            }
        }
        ...
        private void IncrementPercBoost(StatsType stat, int amount)
        {
            IncrementBoost(stat, amount);
        }

        private void IncrementLBBoost(int amount)
        {
            _boost[10] += amount;
        }
