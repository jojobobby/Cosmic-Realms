    ...
    public class Projectile : Entity
    {
        ...
        public Position GetPosition(long elapsedTicks)
        {
            ...
            else
            {
                ...
                else if (ProjDesc.BlazingBoomerang)
                {
                    var d = (ProjDesc.LifetimeMS * ProjDesc.Speed / 10000.0) / 1.25;
                    if (dist > d)
                    {
                        dist = (d - (dist - d));
                    }
                    Angle += (float) dist / 500;
                }

                else if (ProjDesc.VargoSpellBoomerang)
                {
                    var xVal = (ProjDesc.LifetimeMS * ProjDesc.Speed / 10000.0) / 4;
                    if (dist > 2 * xVal)
                        dist -= 2 * xVal;
                    else if (dist > xVal)
                        dist = xVal - (dist - xVal);
