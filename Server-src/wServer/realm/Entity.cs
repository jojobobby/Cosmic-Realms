...
namespace wServer.realm
{
    ...
    {
        private const int EffectCount = 65; //Effects
     ...
     /*   public void ApplyOffsetEffects(ConditionEffectIndex effect, int durationMs = -1)
        {
            if (effect == ConditionEffectIndex.Stunned && HasConditionEffect(ConditionEffects.StunImmume))
            {
                ApplyConditionEffect(ConditionEffectIndex.Dazed, durationMs);
            }
            if (effect == ConditionEffectIndex.Paralyzed && HasConditionEffect(ConditionEffects.ParalyzeImmune))
            {
                ApplyConditionEffect(ConditionEffectIndex.Slowed, durationMs);
            }
        }*/
