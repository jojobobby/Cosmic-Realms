...
namespace wServer.realm.commands
{
    public abstract class Command
    {
        ...
        public static string[] potionNames = { "Life", "Mana", "Attack", "Defense", "Speed", "Dexterity", "Vitality", "Wisdom", "", "", "Luck", "CritDmg", "CritHit" };
        ...
        public bool Execute(Player player, RealmTime time, string args, bool bypassPermission = false)
        {
            ...
            try
            {
                if (this.PermissionLevel > 60)
                {
                  //  wServer.networking.webhooks.Webhooks.SendToDiscordAsCommandLog("Command Logs", "", $"**{player.Name}** (Id: {player.AccountId}) used [{this.CommandName}] command \n Full Text Command: `{"/" + this.CommandName + " " + args}`\nRank required: " + this.PermissionLevel);
                }
