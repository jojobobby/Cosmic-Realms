    ...
    class Shoot : CycleBehavior
    {
        ...
        protected override void TickCore(Entity host, RealmTime time, ref object state)
        {
            ...
            if (cool <= 0)
            {
                ...
                {
                    var desc = host.ObjectDesc.Projectiles[0];
                    var pIndex = _projectileIndex + 1;
                    if (pIndex > host.ObjectDesc.Projectiles.Length)
                        desc = host.ObjectDesc.Projectiles[0];
                    else
                        desc = host.ObjectDesc.Projectiles[_projectileIndex];
