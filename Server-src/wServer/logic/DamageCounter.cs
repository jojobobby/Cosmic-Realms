...
namespace wServer.logic
{
    public class DamageCounter
    {
        ...
        public void Death(RealmTime time)
        {
            ...
            if (enemy.Owner is Moon)
                (enemy.Owner as Moon).OnEnemyKilled(enemy, (Parent ?? this).LastHitter);
                ...
                .Where(p => enemy.Dist(p) < 25))
            {
                ...
                if (upperLimit < xp)
                    ...
                    playerXp = xp;
                int? EnemyExpXML = enemy.ObjectDesc.ExpAmount;

                if (player.Level >= 20)
                    playerXp += Convert.ToInt32(EnemyExpXML);
